/**
 * V7.5.1 E2E: V1.0 real user-path gating tests.
 *
 * Covers 6 critical user paths:
 ...
 *   P1-T04: Repair document preview (page-asset rebuild + image reextract)
 *   P1-T05: Non-PDF material defaults to structured text mode
...
function tag() {
  return `v751-${Date.now()}-${Math.floor(Math.random() * 100_000)}`
}
...
test('P1-T01: upload PDF → parse → preview original page → ask AI', async ({ page, request }, testInfo) => {
  ...
  await expect(page.getByAltText(/原页图像/).first()).toBeVisible({ timeout: 30_000 })
  ...
  await expect(page.locator('.chunk-card, .doc-chunk')).toHaveCount(await page.locator('.chunk-card, .doc-chunk').count())
...
test('P1-T02: generate knowledge points → click KP → verify focus banner', async ({ page, request }, testInfo) => {
  ...
  await expect(page.locator('.kp-focus-banner')).toBeVisible({ timeout: 15_000 })
...
test('P1-T05: non-PDF material defaults to structured text mode', async ({ page, request }, testInfo) => {
  ...
  // Should default to structured text mode (not page mode)
  const radioGroup = page.getByRole('radiogroup', { name: '资料展示模式' })
  if (await radioGroup.isVisible()) {
    // The "结构化文本" option should be selected (or page mode should not be available)
    const cleanRadio = radioGroup.getByText('结构化文本')
