...
- 检索：基于关键词的切块检索（标题 3x / 材料名 2x / 正文 1x 加权打分），支持检索结果可视化（命中切块与得分）；`vector_search` 为预留接口，暂未启用向量召回
- RAG 问答：关键词检索 + 引用校验，基于课程资料的多轮对话式问答，答案附带引用
...
| `LLM_CONFIG_SECRET_KEY`      | 加密用户级 API Key 的 Fernet 密钥          | `change-me-please`              |
| `CORS_ORIGINS`               | 逗号分隔的允许 CORS 来源                   | `http://localhost:5173,...`     |
...
1. **设置 `ENVIRONMENT=production`**：在 `.env` 中将 `ENVIRONMENT` 设为
   `production`。启动时 `validate_prod_secrets()` 会检查密钥是否已更换，
   若仍使用默认值则直接拒绝启动（fail-fast）。
2. **更换 `JWT_SECRET_KEY`**：使用一段足够长的随机字符串（如
   `python -c "import secrets; print(secrets.token_urlsafe(48))"`）。
3. **更换 `LLM_CONFIG_SECRET_KEY`**：使用 Fernet 兼容密钥（如
   `python -c "from cryptography.fernet import Fernet; print(Fernet.generate_key())"`）。
4. **收紧 `CORS_ORIGINS`**：设为实际前端域名，如
   `https://your-domain.com`，切勿在生产环境使用 `*`。
5. **数据库**：生产环境建议使用 PostgreSQL/MySQL，并定期备份。
6. **上传目录**：确保 `UPLOAD_DIR` / `PARSED_DIR` 指向持久化存储且有
   足够磁盘空间。
7. **HTTPS**：在生产环境通过反向代理（Nginx / Caddy）启用 HTTPS。

## CI 持续集成

项目通过 GitHub Actions（配置见 `.github/workflows/ci.yml`）在
每次 push 到 `main` 分支或提交 pull request 时自动执行以下三个
job：
...
- **frontend-build**：在 Ubuntu + Node 20 环境下执行
  ...
  确保前端类型正确且可成功构建。
- **acceptance**：依赖前两个 job 成功后运行
  `bash ./scripts/verify_phase2_engineering.sh`，执行工程验收脚本
  （含主观指标残留、algorithmic-art 移除、生产硬化、字段一致性等
  检查）。

每个 job 都会通过 `actions/upload-artifact` 上传运行结果文件
（`backend-test-result.txt`、`frontend-build-result.txt`、
`acceptance-result.txt`），可在 Actions 运行页面下载，作为验收证据。
...
推送到 `main` 后 GitHub Actions 会自动运行 backend pytest、
