#!/usr/bin/env python
"""Run the secret-safe, isolated real-provider RC acceptance suite.

The API key is read only from ``REAL_LLM_API_KEY``.  This script deliberately
starts its own API, worker, SQLite database, uploads and parsed directories;
it neither reads nor changes a developer's regular course database.  It
returns a non-zero status for mock/fallback/degraded outcomes even if a route
otherwise returned a successful response.
"""
...
def _scenario(name: str, fn, results: list[dict]) -> None:
    ...
    try:
        detail = fn()
        results.append({"id": name, "status": "passed", "latency_ms": int((time.monotonic() - started) * 1000), "detail": detail})
    except Exception as exc:  # preserved as sanitized evidence, then fail the run
...
def _start_process(args: list[str], env: dict[str, str], log_path: Path) -> subprocess.Popen:
    ...
    return subprocess.Popen(args, cwd=ROOT, env=env, stdout=handle, stderr=subprocess.STDOUT, creationflags=creationflags)
...
def _stop_process(process: subprocess.Popen | None) -> None:
    if process is None or process.poll() is not None:
        return
    if os.name == "nt":
        # The venv launcher can hand off to a second python.exe on Windows.
        # Killing only the launcher leaks uvicorn/worker children, so end the
        # tree rooted at the PID this harness created (never discover or touch
...
def _scenario_suite(client: httpx.Client, key: str, provider: str, base_url: str, model: str, scenarios: list[dict], audit_runs: list[dict]) -> None:
    ...
    def chat() -> dict:
        ...
        if any(not citation.get("material_public_id") == material_public_id for citation in data["citations"]):
...
def _configure(client: httpx.Client, username: str, password: str, key: str, provider: str, base_url: str, model: str) -> dict:
    ...
    test = _request(client, "POST", f"/api/v1/llm-configs/{config['id']}/test")
...
def run(args: argparse.Namespace) -> int:
    ...
    env.update({
        ...
        "DATABASE_URL": f"sqlite:///{(runtime_dir / 'acceptance.db').as_posix()}",
        "UPLOAD_DIR": str(runtime_dir / "uploads"),
        "PARSED_DIR": str(runtime_dir / "parsed"),
        "LLM_PROVIDER": "mock",  # user config is the only valid real path
        "REAL_LLM_ACCEPTANCE_MODE": "true",
        "REAL_LLM_ACCEPTANCE_RUN_ID": run_id,
        "REAL_LLM_ACCEPTANCE_OUTPUT_DIR": str(artifact_dir),
        "APP_GIT_COMMIT": _git_sha(),
    })
    backend = worker = None
    scenarios: list[dict] = []
