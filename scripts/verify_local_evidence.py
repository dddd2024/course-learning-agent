...
def verify_manifest(manifest: dict[str, Any], project_dir: Path, verify_state_summaries: bool = False) -> list[str]:
    errors: list[str] = []
    ...
    if errors:
        return errors
    try:
        if git_rev_parse("HEAD", project_dir) != git_rev_parse(manifest["head_sha"], project_dir):
            errors.append("head SHA does not match current Git HEAD")
...
def main() -> None:
    ...
    try:
        ...
        errors = verify_manifest(manifest, project_dir, args.verify_state_summaries)
